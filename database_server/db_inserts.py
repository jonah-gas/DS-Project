...
A more efficient way to insert many (potentially) new rows into a table would be to COPY FROM a csv file into a temporary table,
then write a query which copies only new rows into the main table. (Or use SQLAlchemy's python interface).
...
def _perform_batch_insert(df, table_name, colnames=None, constraint_name=None, batchsize=1, conn=None):
    ...
    for i, row in enumerate(df.itertuples(index=False)):
        ...
        # insert if batchsize is reached
        if (i+1) % batchsize == 0:
...
def _process_individual_insert(conn, cursor, table_name, query_str, batchsize, n_ins, n_failed):
    """Utility function. Process a single (batch) insert query."""
    try:
